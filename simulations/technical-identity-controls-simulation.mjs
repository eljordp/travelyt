...
check("provider return exposes current status", verificationRequest.includes("export async function GET") && verificationRequest.includes('rateLimit(request, "identity:status"') && profile.includes('params.get("identity") !== "return"') && profile.includes('method: "GET"') && profile.includes('setTab("settings")'), "The Stripe return opens Identity Settings and reads the authenticated server status without collecting consent a second time.");
...
check("private object deletion", retention.includes("IDENTITY_ORIGINALS_BUCKET") && retention.includes(".remove(deletionPaths)"), "Deletion removes the claimed and prefix-reconciled private storage objects, not only database references.");
